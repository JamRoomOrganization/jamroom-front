...
export function useRoomQueue(roomId: string): UseRoomQueueResult {
    ...
    // Para evitar actualizaciones innecesarias de la cola
    ...
    const fetchQueue = useCallback(async () => {
        ...
        try {
            ...
            // Crear hash de la cola para detectar cambios reales
            const queueHash = items.map(i => `${i.track_id}-${i.position}`).join(',');

            // Solo actualizar si hay cambios reales
            if (queueHash === lastQueueHashRef.current) {
                console.log("[useRoomQueue] Cola sin cambios, evitando actualización");
                return;
            }
            ...
            const tracksWithMetadata: Track[] = await Promise.all(
                items.map(async (item) => {
                    try {
                        ...
                        // Función helper para obtener el mejor artwork
                        const getArtwork = () => {
                            if (!trackMetadata?.artwork) return undefined;
                            return trackMetadata.artwork['480x480'] ||
                                   trackMetadata.artwork['1000x1000'] ||
                                   trackMetadata.artwork['150x150'];
                        };
    ...
    // Polling cada 20 segundos para sincronizar la cola (reducido para evitar recargas)
    useEffect(() => {
        // Agregar un pequeño delay aleatorio para evitar que todas las pestañas hagan polling al mismo tiempo
        const randomDelay = Math.random() * 3000; // 0-3 segundos
        ...
        const startPolling = setTimeout(() => {
            ...
            }, 20000); // 20 segundos (aumentado de 15000)
