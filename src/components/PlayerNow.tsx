...
type PlayerNowProps = {
  ...
  canControlPlayback?: boolean; // 👈 nuevo
};
...
const PlayerNow = React.memo(function PlayerNow({
  ...
  hasUserInteracted,
  forcePlay,
  canControlPlayback = true, // por defecto true para no romper otros usos
}: PlayerNowProps) {
  ...
  React.useEffect(() => {
    ...
    const handleStalled = () => {
      console.warn(
        "[PlayerNow] Audio detenido (stalled), intentando reanudar"
      );
      if (!audio.paused && isPlaying) {
        audio.load();
        audio.play().catch((err) => {
          console.error(
            "[PlayerNow] Error al reanudar tras stalled:",
            err
          );
        });
      }
  ...
  // Volumen (no lo atamos al permiso de reproducción)
  ...
  const skipTime = (seconds: number) => {
    ...
    audio.currentTime = Math.max(
      0,
      Math.min(total, audio.currentTime + seconds)
    );
  };
        ...
        <div
          ...
          onClick={async () => {
            ...
            try {
              await audio.play();
              console.log(
                "[PlayerNow] ✓ Reproducción iniciada (sin delay)"
              );
